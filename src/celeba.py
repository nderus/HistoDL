#from https://github.com/EleMisi/ConditionalVAE/blob/2025b787097255077c7f568bbadb8bc16e746d72/src/celeba.py
...
class CelebADataset(Sequence):
    ...
    def next_batch_test(self, idx):
        ...
        batch_img_ids = [x[1] for x in self.test_img_ids[idx * self.batch_size : (idx + 1) * self.batch_size]]
        images_id = [x[0] for x in self.test_img_ids[idx * self.batch_size : (idx + 1) * self.batch_size]]
        batch_imgs = self.get_images(images_id) 
    ...
    def get_images(self,imgs_id):
        ...
        for i in imgs_id:
            image_path ='datasets/img_align_celeba/img_align_celeba/' + i
    ...
    def __getitem__(self, index):
        train = self.next_batch(index)
        test = self.next_batch_test(index)
        return train, test
